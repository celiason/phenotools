...
# system.time(nexs2 <- lapply(files[1], ReadMorphNexus))
...
pdf(file = "figs/dupclique_big.pdf")
plot(induced_subgraph(g, v=com$membership==which.max(com$csize)))
dev.off()

pdf(file = "figs/dupclique_small.pdf")
...
dfsum$charpart <- factor(dfsum$charpart, levels = c("skull", "axial", "pectoral", "forelimb", "pelvic", "hindlimb", "muscle"))

ggplot(filter(dfsum, charpart != "muscle"), aes(x = n)) +
...
ggsave("figs/dupcliques.pdf", width = 6, height = 4)
...
df <- filter(df, charpart != "muscle")

ggplot(data=NULL) +
  geom_point(data=df, aes(year, group, color = charpart), cex=1) +
  geom_path(data=df, aes(year, group, color = charpart), alpha = 0.5) +
  facet_wrap(~charpart, scales="free_y") +
  theme_bw() +
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), panel.grid = element_blank(), legend.position = "none", strip.background = element_blank(), strip.text = element_blank()) +
  labs(x = "Year", y = "Duplicated character")

ggsave("figs/character_fate.pdf", width=6, height=3.5)



# df2 <- df %>% group_by(charpart, group) %>% mutate(fad = min(year), lad = max(year))

df2 <- df %>% group_by(charpart, group) %>% summarize(fad = min(year), lad = max(year))
df2$charpart <- droplevels(df2$charpart)
df2$group <- droplevels(df2$group)
df2 <- filter(df2, charpart != "muscle")

# to make a phylogeny, we would want to pick earliest characters in each 'clique'
# use that as the ancestor
# descendents would then be duplicates (again, ordered by time)

fad <- as.matrix(table(df2$fad, df2$charpart))

lad <- as.matrix(table(df2$lad, df2$charpart))

dt.fad <- diff(as.numeric(rownames(fad)))
dt.lad <- diff(as.numeric(rownames(lad)))

# speciation rates
sp.rate <- setNames(sapply(1:ncol(fad), function(x) {mean(fad[-1, x] / dt.fad)}),
    colnames(fad))

ex.rate <- setNames(sapply(1:ncol(lad), function(x) {mean(lad[-1, x] / dt.lad)}),
    colnames(lad))

charpart <- factor(colnames(fad), levels = levels(df2$charpart))

ex.rate
sp.rate

ggplot(data = NULL, aes(x = ex.rate, y = sp.rate, fill = charpart)) +
  geom_point(pch = 21, cex = 3) +
  geom_abline(slope=1, intercept=0, lty = 2) +
  labs(x = "'Extinction rate' (characters per year)", y = "'Speciation rate' (characters per year)") +
  theme(legend.position = "none")

ggsave(file = "figs/characters_speciation_rates.pdf", width = 4, height = 4)





# time series plots of characters

df <- data.frame(char=as.numeric(as.character(final$charnum)), charpart = final$charpart,
  year = as.numeric(str_extract(final$file, "\\d{4,4}$")))

df <- df[!is.na(df$year), ]

df$type <- ifelse(df$charpart %in% c('forelimb', 'pectoral', 'axial', 'hindlimb', 'pelvic', 'skull'), 'bone', 'notbone')

df <- subset(df, type=="bone")

df$charpart <- droplevels(df$charpart)

library(ggdendro)
library(gridExtra)

ts <- as.matrix(table(df$year, df$charpart))
ts <- cbind(year=as.numeric(rownames(ts)), ts)
# log transform non-zeros
ts[,-1][ts[,-1]>0] <- log(ts[,-1][ts[,-1]>0])
# ts[, -1] <- ts[, -1]^.25  # scale because of livezey zusi 2006
ts[, -1] <- scale(ts[, -1])

hc <- hclust(dist(t(ts[,-1])))
hcdata <- dendro_data(hc)
names_order <- hcdata$labels$label

df2 <- as.data.frame(cbind(time=ts[,1], ts[, rev(as.character(names_order))]))

df2 <- melt(df2, id.var="time")

p1 <- ggdendrogram(hcdata, rotate=TRUE, leaf_labels=FALSE) + theme(text = element_text(size = 10))
...
pdf(file = "figs/character time series.pdf", width = 6, height = 5)
grid.arrange(p2, p1, nrow=1, widths = c(3, 2))
dev.off()
```
...
# tmp$taxlabels[which(tmp$taxlabels %in% fossils)] <- paste0('†', tmp$taxlabels[which(tmp$taxlabels %in% fossils)])
...
skull <- na.omit(subset(allnex2, charpartition == 'skull'))

pdf(file="figs/skull_plus_phylo.pdf", width=6, height=4)
plot(skull, phy = tre, bw = FALSE, fsize = 8, na.value = "gray")
dev.off()
