...
## Sorting based on body region (uses REGEX searching)
...
# load REGEX search terms
# terms <- read.csv("data/phenome_terms.csv", as.is=TRUE)
terms <- read_excel("data/phenome terms.xlsx", sheet=1)

# read_excel("data/phenome terms.xlsx", sheet=2)

matches <- lapply(terms$search.term, grep, x = tomatch, perl = TRUE)
length(unlist(matches))/length(tomatch)
length(tomatch[-unlist(matches)])  # 145/2021, ~8% not matched
...
# order in original dataset
# charid <- final$charnum
# final <- final[, order(final$charpart, dup)]
nistid <- which(!1:ncol(final$data) %in% as.numeric(names(tomatch)))

# last in charpartition (if can't be found with match)
# final$charpart[id]
missid <- which(!1:ncol(final$data) %in% c(sortid, nistid))

#combine ids
id <- c(sortid,nistid,missid)
# which(is.na(final$charpart))
# final[, sortid]$charlab
# charpartid <- final$charpart

# all.equal(length(charpartid), length(id), length(dupid))

# reorder by bone
final <- final[, id]

# reorder by character partition
