...
read.nex <- function(file, missing = '?', gap = '-') {
	...
	if (length(grep('CHARLABELS', x, ignore.case=TRUE)) > 0) {
		charlabelsstart <- grep('CHARLABELS', x, ignore.case=TRUE) + 1
	...
	# get state labels
	if (length(grep('STATELABELS', x, ignore.case=TRUE)) > 0) {
		statelabelsstart <- grep('STATELABELS', x, ignore.case=TRUE) + 1
